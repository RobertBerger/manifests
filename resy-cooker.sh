...
# --> yocto-ampliphy-master-2026-05-14-01-pinned-yocto-ampliphy-pinned-distro_ampliphy-resy-systemd-machine_phyboard-pollux-imx8mp-3
MYMAP[yocto-ampliphy-master-2026-05-14-01-pinned-yocto-ampliphy-pinned-distro_ampliphy-resy-systemd-machine_phyboard-pollux-imx8mp-3]="core-image-minimal core-image-sato-sdk"
# <-- yocto-ampliphy-master-2026-05-14-01-pinned-yocto-ampliphy-pinned-distro_ampliphy-resy-systemd-machine_phyboard-pollux-imx8mp-3

# --> yocto-ampliphy-master-2026-08-11-01-pinned-yocto-ampliphy-pinned-distro_ampliphy-resy-systemd-machine_phyboard-pollux-imx8mp-3
MYMAP[yocto-ampliphy-master-2026-08-11-01-pinned-yocto-ampliphy-pinned-distro_ampliphy-resy-systemd-machine_phyboard-pollux-imx8mp-3]="core-image-minimal core-image-sato-sdk"
# <-- yocto-ampliphy-master-2026-08-11-01-pinned-yocto-ampliphy-pinned-distro_ampliphy-resy-systemd-machine_phyboard-pollux-imx8mp-3

# --> yocto-ampliphy-master-2026-08-18-01-pinned-yocto-ampliphy-pinned-distro_ampliphy-resy-systemd-machine_phyboard-pollux-imx8mp-3
MYMAP[yocto-ampliphy-master-2026-08-18-01-pinned-yocto-ampliphy-pinned-distro_ampliphy-resy-systemd-machine_phyboard-pollux-imx8mp-3]="core-image-minimal core-image-sato-sdk"
# <-- yocto-ampliphy-master-2026-08-18-01-pinned-yocto-ampliphy-pinned-distro_ampliphy-resy-systemd-machine_phyboard-pollux-imx8mp-3
...
#read r
 ...
 # x86-64 tensorflow master container e.g. for tensorflow development and testing
  ...
  if [ "$machine" == "yocto-ampliphy-master-2026-05-14-01-pinned-yocto-ampliphy-pinned-distro_ampliphy-resy-systemd-machine_phyboard-pollux-imx8mp-3" ]; then
     cd /workdir/build/yocto-ampliphy-master-2026-05-14-01-pinned/bitbake-builds/yocto-ampliphy-pinned/build
     export BUILDDIR=$(pwd)
     source init-build-env
  fi
  ...
  if [ "$machine" == "yocto-ampliphy-master-2026-08-11-01-pinned-yocto-ampliphy-pinned-distro_ampliphy-resy-systemd-machine_phyboard-pollux-imx8mp-3" ]; then
     cd /workdir/build/yocto-ampliphy-master-2026-08-11-01-pinned/bitbake-builds/yocto-ampliphy-pinned/build
     export BUILDDIR=$(pwd)
     source init-build-env
  fi
  ...
  if [ "$machine" == "yocto-ampliphy-master-2026-08-18-01-pinned-yocto-ampliphy-pinned-distro_ampliphy-resy-systemd-machine_phyboard-pollux-imx8mp-3" ]; then
     cd /workdir/build/yocto-ampliphy-master-2026-08-18-01-pinned/bitbake-builds/yocto-ampliphy-pinned/build
     export BUILDDIR=$(pwd)
     source init-build-env
  fi
     ...
     set ${MYMAP["${1}"]}
       ...
       do
        ...
        if [[ $WORKSPACE = *jenkins* ]]; then
           ...
           if bitbake $var ; then  printf "\e[32m+ (1) $? jenkins, no ERRORS found, bitbake $var\e[0m\n"; else printf "\e[31m+ (1) $? jenkins, ERRORS found, bitbake $var\e[0m\n" && exit 1; fi
        else
           echo "+ (1) bitbake $var"
           #bitbake $var
           if bitbake $var ; then  printf "\e[32m+ (1) $? no jenkins, no ERRORS found, bitbake $var\e[0m\n"; else printf "\e[31m+ (1) $? no jenkins, ERRORS found, bitbake $var\e[0m\n"; fi
        fi
       done
  else # BUILD_ALL != yes
     ${WORKDIR}/killall_bitbake.sh
...
if [ "$#" -eq "2" ]; then
   ...
   if [[ $WORKSPACE = *jenkins* ]]; then
      ...
      if bitbake $2 ; then  printf "\e[32m+ (2) $? jenkins, no ERRORS found, bitbake $2[0m\n"; else printf "\e[31m+ (2) $? jenkins, ERRORS found, bitbake $2\e[0m\n" && exit 1; fi
   else
      echo "+ (2) bitbake $2"
      #bitbake $2
      if bitbake $2 ; then  printf "\e[32m+ (2) $? no jenkins, no ERRORS found, bitbake $2\e[0m\n"; else printf "\e[31m+ (2) $? no jenkins, ERRORS found, WORKSPACE: $WORKSPACE, bitbake $2\e[0m\n" && exit 1; fi
...
# ./resy-poky-container.sh multi-v7-ml 'core-image-sato-sdk -c populate_sdk'
  ...
  if [[ $WORKSPACE = *jenkins* ]]; then
     ...
     if bitbake $@ ; then  printf "\e[32m+ (3) $? jenkins, no ERRORS found, bitbake $@\e[0m\n"; else printf "\e[31m+ (3) $? jenkins, ERRORS found, bitbake $@\e[0m\n" && exit 1; fi
  else
     echo "+ (3) bitbake $@"
     #bitbake $@
     if bitbake $@ ; then  printf "\e[32m+ (3) $? no jenkins, no ERRORS found, bitbake $@\e[0m\n"; else printf "\e[31m+ (3) $? no jenkins, ERRORS found, bitbake $@\e[0m\n"; fi
  fi
fi
