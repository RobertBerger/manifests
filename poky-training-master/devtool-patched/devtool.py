...
class DevtoolAddTests(DevtoolBase):
    ...
    def test_devtool_add_npm(self):
        ...
        #testrecipe = 'savoirfairelinux-node-server-example'
        #bb_vars = get_bb_vars(['FILE'], )
        ...
        # result = runCmd('devtool finish -f %s meta-selftest' % pn)
        # Clean up anything in the workdir/sysroot/sstate cache (have to do this *after* devtool add since the recipe only exists then)
        bitbake('%s -c cleansstate' % pn)
        # Test devtool build
        result = runCmd('devtool build %s' % pn)
        ...
        result = runCmd('devtool finish -f %s meta-selftest' % pn)
        self.add_command_to_tearDown('bitbake -c cleansstate %s' % pn)
        self.add_command_to_tearDown('bitbake %s' % pn)


    def test_devtool_add_npm_two(self):
        collections = get_bb_var('BBFILE_COLLECTIONS').split()
        if "openembedded-layer" not in collections:
            self.skipTest("Test needs meta-oe for nodejs")

        pn = 'hello-international'
        pv = '2.2.1'
        url = 'npm://registry.npmjs.org;package=hello-international;version=' + pv
        # Test devtool add
        self.track_for_cleanup(self.workspacedir)
        self.add_command_to_tearDown('bitbake -c cleansstate %s' % pn)
        self.add_command_to_tearDown('bitbake-layers remove-layer */workspace')
